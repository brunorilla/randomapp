...
export const Board: FC<BoardProps> = (props: PropsWithChildren<BoardProps>) => {
    ...
    useEffect(() => {
        ...
        if (win !== null) {
            setWinner(win === Players.One ? Players.One : Players.Two)
            setPlayers((prevPlayers) => ({
                ...prevPlayers,
                [winner === Players.One ? "player1" : "player2"]: {
                    ...prevPlayers[winner === Players.One ? "player1" : "player2"],
                    points: prevPlayers[winner === Players.One ? "player1" : "player2"].points + 1
                }
            }));
        }
    }, [turn, boardState]);
    ...
    return (
        <>
            ...
            {turn === null ? (
                ...
                <>
                    <Header style={{ backgroundColor: "white" }}>
                        <h2>THE WINNER IS: {winner === Players.One ? `${players.player1.name}` : `${players.player2.name}`}</h2>
